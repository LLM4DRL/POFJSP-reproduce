...
This is a unified implementation that combines all components into a single module
...
class CrossoverOperator:
    ...
    def _combine_sequences(self, seq1: List[Operation], seq2: List[Operation], 
        ...
        # Simple combination: alternate between sequences where valid
        combined = []
        used_ops = set()
...
class MutationOperator:
    ...
    def _can_swap_operations(self, op1: Operation, op2: Operation, 
        ...
        # Check if swapping violates precedence constraints
...
class IAOAGNSAlgorithm:
    ...
    def solve(self, problem: ProblemInstance, verbose: bool = False) -> Solution:
        ...
        try:
            ...
            # Check convergence
            final_improvement = (initial_makespan - best_solution.makespan) / initial_makespan
            if final_improvement < 0.01:  # Less than 1% improvement
    ...
    def get_algorithm_info(self) -> dict:
        """Get information about algorithm configuration and components."""
        return {
            'algorithm': 'IAOA+GNS',
            'version': 'unified',
            'config': {
                ...
                'moa_range': [self.config.moa_min, self.config.moa_max]
            },
            'components': [
                'PopulationManager',
                'CrossoverOperator', 
...
def solve_with_iaoa_gns(problem: ProblemInstance, 
                       pop_size: int = 80, 
                       max_iterations: int = 60,
                       verbose: bool = False) -> Solution:
    """
    Backward compatible function for solving with IAOA+GNS.
    ...
    return algorithm.solve(problem, verbose)