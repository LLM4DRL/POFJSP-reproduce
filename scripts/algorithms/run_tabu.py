#!/usr/bin/env python3
"""
Standalone Tabu Search Runner for POFJSP

This script provides a command-line interface to run the Tabu Search
...
from src.algorithms.tabu_search import solve_with_tabu, TabuParams
...
def solve_single_instance(problem_path: str, output_dir: str, tabu_params: TabuParams, verbose: bool = False) -> Dict:
    """Solve a single POFJSP instance with Tabu Search."""
    ...
    if verbose:
        print(f"Solving {problem_name} with Tabu Search...")
    ...
    result = solve_with_tabu(problem, tabu_params, verbose=verbose)
    ...
    solution_data = {
        ...
        'tabu_parameters': {
            'max_iterations': tabu_params.max_iterations,
            'tabu_list_size': tabu_params.tabu_list_size,
            'max_stagnation': tabu_params.max_stagnation,
            'neighborhood_size': tabu_params.neighborhood_size,
            'aspiration_criterion': tabu_params.aspiration_criterion
        }
    }
    
    output_file = os.path.join(output_dir, f"{problem_name}_tabu_solution.json")
...
def solve_dataset(dataset_dir: str, output_dir: str, tabu_params: TabuParams, verbose: bool = False) -> Dict:
    ...
    summary_file = os.path.join(output_dir, "tabu_summary.json")
...
def main():
    """Main CLI interface."""
    parser = argparse.ArgumentParser(description="Run Tabu Search for POFJSP")
    
    # Required arguments
    parser.add_argument("problem", type=str, help="Path to problem file or dataset directory")
    
    # Optional arguments
    parser.add_argument("--output-dir", type=str, default="./outputs/tabu",
                        help="Output directory for results")
    parser.add_argument("--max-iterations", type=int, default=1000,
                        help="Maximum iterations")
    parser.add_argument("--tabu-list-size", type=int, default=50,
                        help="Tabu list size")
    parser.add_argument("--max-stagnation", type=int, default=100,
                        help="Maximum iterations without improvement")
    parser.add_argument("--neighborhood-size", type=int, default=30,
                        help="Neighborhood size")
    parser.add_argument("--aspiration-criterion", action="store_true", default=True,
                        help="Enable aspiration criterion")
    parser.add_argument("--intensification-threshold", type=int, default=20,
                        help="Intensification threshold")
    parser.add_argument("--diversification-threshold", type=int, default=50,
                        help="Diversification threshold")
    ...
    # Create Tabu parameters
    tabu_params = TabuParams(
        max_iterations=args.max_iterations,
        tabu_list_size=args.tabu_list_size,
        max_stagnation=args.max_stagnation,
        neighborhood_size=args.neighborhood_size,
        aspiration_criterion=args.aspiration_criterion,
        intensification_threshold=args.intensification_threshold,
        diversification_threshold=args.diversification_threshold
    )
    
    # Run appropriate mode
    if args.dataset_mode:
        solve_dataset(args.problem, args.output_dir, tabu_params, args.verbose)
    else:
        solve_single_instance(args.problem, args.output_dir, tabu_params, args.verbose)
